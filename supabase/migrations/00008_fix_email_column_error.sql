-- Fix email column error in user creation trigger

-- Create function to handle new user creation without email column reference
...
BEGIN
  ...
  -- Use email from auth.users table, not from public.users
  ...
  -- Note: public.users table does NOT have an email column
